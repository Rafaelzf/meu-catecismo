...
function FormEditTopic() {
  ...
  const { data, isValidating, error } = useSWR(
    currentTopic ? `questionAsks-${currentTopic.id}` : null,
  ...
  const defaultSendValues = {
    ...
    questionsAsks: dataQuestionsAsks,
  };
  ...
  async function onSubmit(values: z.infer<typeof editFormSchema>) {
    const { title, active, questionsAsks } = values;
      ...
      <form onSubmit={createform.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-end items-center align-middle">
          <Button
            type="button"
            variant="outline"
            className="border-0 text-emerald-500 hover:text-emerald-500 flex justify-end items-center align-middle gap-3"
            onClick={() => addQuestion()}
          >
